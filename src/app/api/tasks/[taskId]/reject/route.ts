...
export async function POST(req: NextRequest, { params }: { params: { taskId: string } }) {
  ...
  const { data: task } = await db
    ...
    .select('id, name, status, storage_path, pic_user_id, content_plan:content_plans!content_plan_id(id, title, created_by)')
    .eq('id', params.taskId)
    .single();
  ...
  // Hapus file RAW dari storage bila ada
  ...
  // Notif ke PIC untuk submit ulang
  if (task.pic_user_id) {
    const noteText = revision_notes ? ` Catatan: ${revision_notes}` : '';
    await sendNotifications({
      userIds: [task.pic_user_id],
      type: 'submission_rejected',
      message: `Task "${task.name}" diminta revisi, silakan submit ulang pada plan "${plan?.title}".${noteText}`,
